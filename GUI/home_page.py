...
class Home_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.info_image = Image.open("assets/info.png").resize((button_diameter // 2, button_diameter // 2), Image.LANCZOS)
        self.competition_image = Image.open("assets/competition.png").resize((button_diameter // 2, button_diameter // 2), Image.LANCZOS)
        self.pattern_image = Image.open("assets/pattern.png").resize((button_diameter // 2, button_diameter // 2), Image.LANCZOS)
        ...
        def create_circular_button(frame, text, command, image):
            ...
            btn_container = tk.Frame(frame, bg=background_color)
            btn_container.pack(side="left", padx=40)

            # Label above the button with extra vertical padding
            btn_label = tk.Label(btn_container, text=text, font=("Arial", 32, "bold"), bg=background_color)
            ...
            canvas = tk.Canvas(btn_container, width=button_diameter, height=button_diameter, highlightthickness=0, bg=background_color)
            canvas.pack()
